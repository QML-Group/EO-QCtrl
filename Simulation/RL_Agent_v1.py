...
model = keras.Sequential([keras.layers.Dense(number_of_timesteps, activation='tanh', input_shape=(len(h_c), number_of_timesteps)), keras.layers.Dense(number_of_timesteps, activation='tanh')])
...
for episode in range(num_episodes):
    ...
    while not done:
        ...
        if np.random.rand() < exploration_rate:  # Start with higher exploration
            ...
            pulses = model.predict(np.array([state]))[0]
            print("Predicted pulses is:", pulses)
        ...
        print("Fit is:", model.fit(np.array([state]), np.array([target]), epochs=1, verbose = 0))
